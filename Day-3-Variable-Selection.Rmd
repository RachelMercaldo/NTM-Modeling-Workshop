...
```{r}
vars <- c("temperature", "precipitation", "pm2p5", "humidity", "wind_speed")

#or

vars2 <- colnames(data[,"fill in the column numbers here"]) #what does colnames() do?
```
...
X <- data %>% 
  dplyr::select(all_of(vars)) #what does the dplyr
...
Now we know what variables are important. We can choose a specific method or set some criteria (such as, final variables must have been selected as important in 2 of the 3 methods) or 
