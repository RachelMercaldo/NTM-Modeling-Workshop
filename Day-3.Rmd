...
#Let's do a test:
test <- data %>%
  dplyr::select(water_bdy) #no negative
...
data <- data %>%
  dplyr::select(fips, month_year, case_rate_100k, avg_temperature_air_2m_f, 
...
num_fips <- unique(data$fips)
length(num_fips)

data2 <- data %>% group_by(month_year) %>%
  summarize(temperature = mean(temperature),
            precipitation = sum(precipitation),
            case_rate_100k = mean(case_rate_100k))


ggplot(data2, aes(x = precipitation, y = case_rate_100k)) +
  geom_point() +
  labs(title = "Case Rate vs Precipitation", x = "Precipitation", y = "Case Rate per 100k")
